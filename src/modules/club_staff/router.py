...
router = APIRouter(prefix="/clubs", tags=['Club Staff Management'])


@router.post("/{club_id}/staff", summary="Add staff member")
async def add_staff(club_id: int, staff_data: SClubStaffAdd, auth=Depends(RoleChecker(allowed_roles=[]))):
...
@router.get("/{club_id}/staff", summary="Get staff list")
async def list_staff(club_id: int, auth = Depends(RoleChecker(allowed_roles=["admin", "manager"]))):
